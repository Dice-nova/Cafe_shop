...
def create_tables():
    cursor.execute("CREATE TABLE food (ID INT NOT NULL PRIMARY KEY, name VARCHAR(30), price INT UNSIGNED NOT NULL, allergens VARCHAR(50), details VARCHAR(350), stock INT UNSIGNED)")
    cursor.execute("CREATE TABLE drink (ID INT NOT NULL PRIMARY KEY, name VARCHAR(30), price INT UNSIGNED NOT NULL, allergens VARCHAR(50), details VARCHAR(350), stock INT UNSIGNED)")
    cursor.execute("CREATE TABLE book (ID INT NOT NULL PRIMARY KEY, name VARCHAR(30), price INT UNSIGNED NOT NULL, author VARCHAR(50), details VARCHAR(350), stock INT UNSIGNED)")

def create_rec():
    base_command = "INSERT INTO food (ID, name, price, allergens, details, stock) VALUES (%s, %s, %s, %s, %s, %s)"
    ID=0
    for item in Food:
